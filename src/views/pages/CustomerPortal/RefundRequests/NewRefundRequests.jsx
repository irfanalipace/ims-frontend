...
const NewRefundRequests = ({ edit }) => {
	...
	const [taxtValue, setTaxValue] = useState('');
	...
	const handleInvoiceChange = async invId => {
		...
		if (invId) {
			try {
				...
				console.log('getInvoiceDashboardDetailsApi resp', resp);
				setTaxValue(resp?.data?.tax_amount);
				setDiscountValue(resp?.data?.discount_value);
				if (edit) {
