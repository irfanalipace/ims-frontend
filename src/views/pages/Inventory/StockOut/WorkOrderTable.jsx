...
const WorkOrderTable = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (resp.status === 'ready_for_delivery') {
        notyf.success('Work order already created');
        navigate('/stock-out');
      }
    } catch (error) {
  ...
  const handleQuantityChange = (e, index) => {
    setApiData(prev => {
      ...
      if (value > 0 && value < parseInt(originalQuantity)) {
        items[index] = {
          ...items[index],
          quantity: value
        };
    ...
    },
    {
      ...
      Cell: ({ cell, row }) => (
        <Box
          ...
          {row?.original?.item?.product_label?.qr_code_url && (
            <img
              src={row?.original?.item?.product_label?.qr_code_url}
              alt='Item Image'
              style={{
                maxWidth: '100%',
                maxHeight: '100%',
        ...
        <DataTableContainer takenHeight='300'>
          <DataTable
            ...
            renderBottomToolbarCustomActions={() => (
              <Grid container justifyContent={'end'} pr={20}>
                <Button
                  disabled={loading}
