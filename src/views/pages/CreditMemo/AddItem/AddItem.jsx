...
const areEqual = (prevProps, nextProps) => {
	...
	return (
		...
		prevProps.taxtValue === nextProps.taxtValue
	);
};
	...
	({
		...
		useEffect(() => {
			...
			const totle_item = isEdit
				? invoicedItem && invoicedItem.length
				: creditMemoState?.invoice_items &&
				  creditMemoState?.invoice_items.length;
			discount_per_item = discountValue / totle_item;
			...
			if (isEdit === true) {
				...
				const updatedState = invoicedItem.map(invoiceItem => {
					...
					const taxPercentage = (taxtValue / invoiceItem.quantity) * 100;

					const newTaxAmount =
						(matchingCreditMemo?.quantity * taxPercentage) / 100;
					let discountPerOneitem = discount_per_item / invoiceItem.quantity;
					discountPerOneitem =
						discountPerOneitem * matchingCreditMemo?.quantity;
					...
					if (matchingCreditMemo) {
						...
						return {
							...
							amount: updatedTotal,
							isChecked: true,
							discount_per_item: discountPerOneitem,
		...
		useEffect(() => {
			if (submitCount > 0 && isValid) {
				...
				formik.setFieldValue(`discount_value`, fixedDiscountAmount);
									...
									<FormField
										...
										onChange={event => {
											...
											if (!isNaN(newQuantity) && !(newQuantity < 1)) {
												...
												let discountPerOneitem =
													row.discount_per_item / row.quantity;
												discountPerOneitem = discountPerOneitem * newQuantity;
												const newTaxAmount =
