...
const BillsAddItem = ({
	...
	const itemNameKey = type === 'invoice' ? 'item_name' : 'item_name';
	const percentKey = type === 'invoice' ? 'percentage' : 'Percentage';
	const dollorKey = type === 'invoice' ? 'amount' : 'dollar';
	const taxOption = type === 'invoice' ? 'tax_type' : 'items_rates_are';
	const taxTypeKey = type === 'invoice' ? 'exclusive' : 'tax_exclusive';
	...
	const labelToTaxAmountMap = values?.items?.reduce((map, currentInvoice) => {
		let invoice;

		invoice = { ...currentInvoice };
		let jsonDetial;
		if (type === 'invoice' && isEdit) {
			if (currentInvoice?.detail) {
				jsonDetial = JSON.parse(JSON.parse(currentInvoice.detail));
				invoice.tax = jsonDetial?.tax;
			}
		}
		if (invoice.tax?.id !== 1) {
	...
	const taxInclusiveOption_ = [
		{
			value: type === 'invoice' ? 'inclusive' : 'tax_inclusive',
			text: 'Tax Inclusive',
		},
		{
			value: type === 'invoice' ? 'exclusive' : 'tax_exclusive',
			text: 'Tax Exclusive',
		},
	];
	...
	useEffect(() => {
		...
		if (customerTax && isEdit === false) {
			...
			invoice_items_copy.forEach(item => {
				if (item?.detail) {
					detail = JSON.parse(JSON.parse(item.detail));
				}
				if (!item?.[itemNameKey]) {
					item.tax = {
						label: 'Non-Taxable',
	...
	const calculateTotal = useCallback(() => {
		...
		let tax_amount = 0;
		if (values[taxOption] === taxTypeKey) {
		...
		let discount = 0;
		if (values.discount_type === percentKey) {
			discount = (subtotal * values.discount) / 100;
		} else if (values.discount_type === dollorKey) {
	...
	const discountCalculate = useCallback(() => {
		return values.discount_type === percentKey
								...
								<Grid item xs={5} display='flex' justifyContent='space-between'>
									<Stack alignItems={'center'} direction='row' spacing={2}>
										...
										<Box sx={{ width: '100px' }}>
											<HoverPopover
												iconSize={25}
												text='Amount spent on shipping the goods'
											/>
										</Box>
									</Stack>
								</Grid>
